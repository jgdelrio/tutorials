...
TESTS = [("aabaa", [[1, 1], [1, 4], [1, 1], [1,4], [0,2]], [1, 8, 1, 8, 5])]
...
class STree():
    ...
    def _create_leaf(self, x, i, u, d):        # string, index, current node, ref
        w = _SNode()
        w.idx = i
        w.depth = len(x) - i
        u._add_transition_link(w, x[i + d])   # add the node created
        w.parent = u
        return w
...
def eval_substring(s, start, end):
    st = STree(s)
    st.find('ba')
    st.find_all('aa')
    st.find('aa')
