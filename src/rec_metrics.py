...
# Keys used in the datasets
KEYS = ['userId', 'itemId', 'score']


def number2string(n):
    """
    Transform number or series into string (or series of strings)
    :param n: number or series
...
def align_metrics(predictions, expected, keys=None):
    """
    ...
    :param predictions: predictions dataset
    :param expected:    validation dataset
    :param keys:        names of the keys that reference 'userId', 'itemId' and 'score' in that order
    ...
    if not isinstance(expected, pd.core.frame.DataFrame):
        raise TypeError("expected must be a pandas dataframe")

    if keys is None:
        keys = KEYS

    expected = expected[keys].copy()
    predictions = predictions[keys].copy()
    predictions.columns = [*keys[:2], 'predictions']

    # Detect missing values
    idx_missing_users = ~expected[keys[0]].isin(predictions[keys[0]].tolist())
    idx_missing_items = ~expected[keys[1]].isin(predictions[keys[1]].tolist())
    if any(idx_missing_users):
        print("\nMissing users in predictions:\n{}".format(set(expected[keys[0]][idx_missing_users].values)))
    if any(idx_missing_items):
        print("\nMissing items in predictions:\n{}".format(set(expected[keys[1]][idx_missing_items].values.tolist())))

    cmp_table = pd.merge(expected,
                         predictions,
                         left_on=keys[:2],
                         right_on=keys[:2],
                         how='left')
...
def drop_repetitions(predictions, expected, keys=None):
    """
    Receive one dataframe for the predictions and one for the test
    Both dataframes have the keys 'user', 'item' and 'rating' in that order

    :param predictions: predictions dataset
    :param expected:    validation dataset against which we verify existing user-item pairs
    :param keys:        names of the keys that reference 'userId', 'itemId', 'score' in that order
    ...
    i2 = expected.set_index(keys[:2]).index
...
def mse(predictions, expected, keys=None, mlflow_logger=None):
    """
...
def rmse(predictions, expected, keys=None, mlflow_logger=None):
    """
...
def error_measures(predictions, expected, keys=None, output='dict', mlflow_logger=None):
    """
...
def precision_recall_at_k(predictions, expected, keys=None, k=10, threshold=0.5,
                          mlflow_logger=None):
    """
...
def get_top_n(predictions, expected=None, n=10, userid=None, output='dataframe'):
    """
    Return the top-N recommendation for each user from a set of predictions.
    :param predictions: dataframe of predictions
    :param expected:    source dataframe. Must be provided if we want only new results in the top 10 predictions.
    ...
    if expected is None:
        ...
        pred = drop_repetitions(predictions, expected)
...
def get_per_fields(df, fields_extract, field_search, val_search):
    """
    Receive a DataFrame from which we to extract rows and columns as per val_search and field_search
    :param df:
    ...
    if not (field_search in df.columns.values):
        raise KeyError('Field not found in dataset')

    idx = df[field_search].isin(val_search)
    df = df[[idx, *fields_extract]].drop_duplicates(idx, keep='first')
...
def catalogue_coverage(catalogue, recommendations,
                       cfield='itemId', rfield=None,
    ...
    :param cfield:          field name in the catalogue (can be itemId, year, cert, etc depending on the evaluation)
    ...
    :param rating_threshold:(optional) threshold to filter the recommendations
    ...
    try:
        ...
        is_na = recommendations[rfield].isna()
    ...
    if r_rating_field is not None:
        rec = recommendations[recommendations[r_rating_field] >= rating_threshold]
    else:
        rec = recommendations

    if rfield not in rec.columns:
        ...
        rec = merge_by(rec, rfield, catalogue, cfield, [cfield])

    try:
        n_recommended_items = rec[rfield].nunique()
    except TypeError:       # If elements within the field contains a list...
        is_na = rec[rfield].isna()
        n_recommended_items = len(set().union(*rec[~is_na][rfield]))
        if any(is_na):
...
def catalogue_coverage_distr(catalogue, recommended, cfield='itemId', rfield=None,
                             r_rating_field=None, rating_threshold=0.5,
